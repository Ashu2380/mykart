...
window.speechSynthesis.speak(utterence)
 ...
 if(transcript.toLowerCase().includes("search") && transcript.toLowerCase().includes("open") && !showSearch){
    ...
    else if(transcript.toLowerCase().includes("chat") || transcript.toLowerCase().includes("assistant") || transcript.toLowerCase().includes("help") || transcript.toLowerCase().includes("support")){
      speak("opening AI shopping assistant")
      // The chatbot is already rendered globally, so just speak
    }
    else{
  ...
  return (
    <div className='ai-container' onClick={()=>{recognition.start();
    openingSound.play()
    setActiveAi(true)
    }}>
